...
c=3e8;
 ...
 Pperiod=2*pi/(k2-2*k1);
 Lcord=dz:dz:Pperiod*119; 
...
%           E2_L = zeros(size(Z2));          
 ...
 %~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          ...
          H0=1; 
          TransFact =sqrt(-1)*2*pi*((n2/lambda2)^2-B).^0.5;
...
E1out=[];
       ...
       flag=0;
       flag1=2943;
    for  z_slice=(-round(flag1):1:round(flag1)).*dz 
      tic
        flag=flag+1;
       xi=2*z_slice/b ;                             
       tau=1/(1+sqrt(-1)*xi);
        ...
        E1 = (E0*tau)*exp(sqrt(-1)*k1*flag*dz).*exp(-(X1.^2+Y1.^2)./(omega0^2).*tau);
%       E1 = -(E0*tau)^2.*exp(sqrt(-1)*k1*z_slice).*exp(-2*Msq./(omega0^2).*tau);
      ...
      % E1out= [E1out; E1(100,:)];
        ...
        Poling_sign=zz(flag);
        dE2_dz=2*pi*(sqrt(-1)*w2/(c*n2))*deff*E1.^2.*Poling_sign;
...
%     end
    toc
    po2w(flag)= n2*c/(8*pi)*sum(abs(E2out(:)).^2)*dx*dy;
